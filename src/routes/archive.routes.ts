...
router.post('/chat-or-comments', async (req: Request, res: Response): Promise<void> => {
   try {
      ...
      try {
         ...
         isLiveStream = true; // Fallback to check chat replay if details API fails
      }
      ...
      if (activeLiveChatId) {
         try {
            ...
            // later on if wanted only streamer chat than you can just filter it from the live messages

            console.log("from archive routes live chat : ",liveMessages);
            res.json({
